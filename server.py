...
server_socket.bind(('192.168.143.181', 8000))  # ADD IP HERE
...
frame_width = 640
frame_height = 480
videoSource = "C:\\Users\\anand\\Documents\\UTAR\\Y3S1\\FYP Project\\Code\\VideoSource\\livestream.mp4"
...
try:
    ...
    while True:
        ...
        frame = np.asarray(image_pil)
        if img is None:
            # img = pl.imshow(image)
            img2 = cv2.imshow('object detection',  frame)
        else:
            img.set_data(frame)
            ...
            # not yet been estimated then estimate it
                ...
                if to.lastPoint and not to.estimated:
                    ...
                    print("[INFO] Speed of the vehicle that just passed"\
                        " is: {:.2f} KMPH {} {}".format(to.speedKMPH, objectID, to.direction))
